...
const FORTE = join(ROOT, 'forte/target/release/forte');
...
try {
  await page.goto(
    `http://127.0.0.1:${STATIC_PORT}/forte/web/hub.html?api=http://127.0.0.1:${API_PORT}`,
    { waitUntil: 'load' }
  );
  ...
  await page.goto(
    `http://127.0.0.1:${STATIC_PORT}/forte/web/?api=http://127.0.0.1:${API_PORT}`,
    { waitUntil: 'load' }
  );
