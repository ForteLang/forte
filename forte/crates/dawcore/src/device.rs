...
use crate::dsp::effects::{Chorus, Compressor, Crush, Drive, Duck, Eq3, Exciter, FdnReverb, Gate, Glue, Limiter, ParComp, Space, Pump, RingMod, Saturate, StereoDelay, Stutter, TapeStop, Transient, Vinyl, Width};
...
pub fn build_dsp(dev: &Device, sr: f32) -> Dsp {
    match dev.kind {
        ...
        DeviceKind::Glue => Dsp::Audio(Box::new(Glue::new(sr))),
...
impl AudioFx for Limiter {
    fn process(&mut self, l: f32, r: f32) -> (f32, f32) {
        Limiter::process(self, l, r)
    }
    fn configure(&mut self, p: &[f32]) {
        if p.len() >= 2 {
...
impl AudioFx for Glue {
    fn process(&mut self, l: f32, r: f32) -> (f32, f32) {
        Glue::process(self, l, r)
    }
    fn configure(&mut self, p: &[f32]) {
        if p.len() >= 8 {
            self.thresh = p[0];
