...
| D-13 | Forte Studio ships as a VS Code extension; DAW surfaces are webview panels wired to `forte viz` / `forte edit` | Maintainer direction: VS Code's git GUI, GitHub plugin and AI assistants must be available in Studio; rebuilding commodity IDE infrastructure is wasted effort | Approved 2026-07-14, **superseded same day by D-14** (the extension survives inside the fork as its bundled core) |
| **D-14** | **Forte Studio is a FORK of VS Code (the Cursor model)** — not a plugin riding inside stock VS Code. We keep VS Code's foundations (editor, terminals, SCM, extension ecosystem, LSP, remote dev) and evolve the workbench itself into a DAW. The fork owns what the extension API cannot express: the **DAW view as the primary editor** for `.forte` files (code and arrangement as one first-class split editor, not a side-panel picture), a **transport bar** (play/stop/position/meters) as a workbench part, DAW-grade layout defaults, product branding, bundled Forte extensions, space-bar-to-play keybindings. Everything an extension CAN express stays in the bundled extension — the patch set on upstream stays as thin as Cursor keeps theirs | Maintainer direction 2026-07-14 (second iteration): "a plugin means the right side only shows visualizations and rich GUI operation is impossible — I want a Cursor-style fork: VS Code's basic machinery, developed into a DAW." The extension prototype proved the projections work but also proved the plugin ceiling: webview panels cannot be the default editor experience, cannot own the workbench layout, cannot put a transport in the workbench chrome. Details in 09-forte-studio-fork.md | **Approved 2026-07-14** |
