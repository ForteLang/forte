...
| `sample Rec = dig("../glitch/song.forte", beats: 16, skip: 16)` | Crate digging: compile and render ANOTHER SONG FILE (its imports, bounces and digs included) with the same deterministic engine, window it by `skip`/`beats`, by the source's own bars (`bars: 5..8`), or by a section NAME the source declares (`section: "drop"`, E-DIG-005 if missing — survives the record being rearranged; defaults: whole record), and register it as a sample asset rooted at `note` (default C3). The +2-beat tail past the window is the record's REAL continuation. Cycles are E-DIG-002; a loaderless build is E-DIG-001. A sampler playing a dig record defaults `end` to the musical edge (beats/(beats+2)) — no tail-fraction magic — and `semis: -5` repitches in semitones (pitch slot 0.5 ± n/48)  Both `bounce` and `dig` press STEREO records — the sampler reads both channels through one voice, so the source's width survives (mono assets are untouched, bit-exact). `key: "E minor"` RE-PRESSES the record in another key before sampling: the source recompiles wrapped in a key-overridden placement, so melody transposes while beat literals stay put (the source must declare its own key — E-DIG-006). `tempo: "match"` recompiles the source at the digging song's tempo, so slice grids line up without granular stretch | Your own songs are the records: dig, chop, repitch, rebuild — and re-key or re-clock them at the source, because the source is code |
