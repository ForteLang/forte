...
| `sample Rec = dig("../glitch/song.forte", beats: 16, skip: 16)` | Crate digging: compile and render ANOTHER SONG FILE (its imports, bounces and digs included) with the same deterministic engine, window it by `skip`/`beats` (defaults: whole record), and register it as a sample asset rooted at `note` (default C3). The +2-beat tail past the window is the record's REAL continuation. Cycles are E-DIG-002; a loaderless build is E-DIG-001. A sampler playing a dig record defaults `end` to the musical edge (beats/(beats+2)) — no tail-fraction magic — and `semis: -5` repitches in semitones (pitch slot 0.5 ± n/48)  Both `bounce` and `dig` press STEREO records — the sampler reads both channels through one voice, so the source's width survives (mono assets are untouched, bit-exact) | Your own songs are the records: dig, chop, repitch, rebuild |
