...
| `kit(C2: kickTake, D2: snareTake, …)` | gain, attack, decay, sustain, release, pitch. Note-name keys assign recorded takes OR bounce samples to pads (an exactly matching pitch plays that pad at original speed) — a pad can be a whole produced MOMENT, a block bounced with its insert chains. A `beat` literal strikes the lowest-pitched pad |
