...
| 1.5 | マイク録音 v1: `.frec`+来歴+クラッシュ回復+ループバック較正 — **🔶 `.frec` フォーマット+来歴強制(来歴なしはコンパイルエラー E-PROV-001)+`import take from "*.frec"`+`audio take at …` 構文+エンジン再生を実装。ブラウザ録音 UI・較正は未** | SRS-REC-002..005 |
