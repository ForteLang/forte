...
- 4.1 系譜ディグ UI(fork ツリー、演奏者横断、モジュール横断「この楽器を使う曲」) — **🔶 fork 家系図を前倒し実装: GET /api/lineage(fork の森 JSON、循環安全)+ hub トップにツリー表示(♪/📚、release・再生数バッジ、クリックで曲ページへ)。多世代ネストを Rust テスト+E2E で検証。演奏者横断・モジュール横断は次段**
